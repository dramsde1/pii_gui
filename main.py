...
output_folder = sg.popup_get_folder('Folder to save images/metadata to', default_path='')
...
def save_to_output(image_name: str):
    #move metadatafile as well as image
    try:
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        break
    elif event in ('Next', 'MouseWheel:Down', 'Down:40', 'Next:34', 'No'):
        i += 1
        if i >= num_files:
            i -= num_files
        ...
        if i < 0:
            i = num_files + i
        filename = os.path.join(folder, fnames[i])
    elif event in ('Yes', 'MouseWheel:Down', 'Down:40', 'Next:34'):
        if filename not in added_files:
            added_files.append(filename)
            save_to_output(filename)
        # same as next
        i += 1
        if i >= num_files:
            i -= num_files
